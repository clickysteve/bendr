...
/* Two ways of naming the same thing, because two kinds of people arrive here.
   Anything a bar or longer is written the way a clock divider writes it - one
   cycle every so many beats - because that is what it is and that is the
   language it is asked for in. Anything shorter than a bar keeps its note
   value, where 1/4 means a quarter note and not a division by four. The
   annotation on every entry says which way round it is, so the two halves of
   the menu cannot be misread for each other. */
const SYNC_DIVS = [
  ["0","FREE"],
  ["512","\u00f7512 \u00b7 128 BAR"], ["256","\u00f7256 \u00b7 64 BAR"],
  ["128","\u00f7128 \u00b7 32 BAR"], ["64","\u00f764 \u00b7 16 BAR"],
  ["32","\u00f732 \u00b7 8 BAR"], ["16","\u00f716 \u00b7 4 BAR"], ["12","\u00f712 \u00b7 3 BAR"],
  ["8","\u00f78 \u00b7 2 BAR"], ["6","\u00f76 \u00b7 1.5 BAR"], ["4","\u00f74 \u00b7 1 BAR"],
  ["3","1/2 DOT"], ["2.6667","1 BAR TRIP"], ["2","1/2"],
  ["1.5","1/4 DOT"], ["1.3333","1/2 TRIP"], ["1","1/4"], ["0.75","1/8 DOT"],
  ["0.6667","1/4 TRIP"], ["0.5","1/8"], ["0.3333","1/8 TRIP"], ["0.25","1/16"],
  ["0.1667","1/16 TRIP"], ["0.125","1/32"], ["0.0833","1/32 TRIP"],
  /* Past a thirty-second the divisions are asking for more cycles per second
     than the render can draw, and what comes out is an alias rather than the
     rate on the label: at 120 BPM a 1/128 wants sixty-four hertz and sixty
     frames a second can carry thirty. That is not a reason to leave them out
     of a machine like this one - a wheel filmed through a shutter turning the
     wrong way is the same effect and it is worth having - but it is a reason
     to say so, which the card does underneath the selector. */
  ["0.0625","1/64"], ["0.0417","1/64 TRIP"], ["0.03125","1/128"],
];
...
/* Under the rate: the period when it is free-running, and when it is locked to
   the tempo the frequency that lock actually works out at, since a division is
   only meaningful once you know the tempo. Above half the frame rate it says
...
function lfoNote(m){
  ...
  if(m.sync > 0){
    let lim = 30;
    try{ lim = (engineRate > 0 ? engineRate : 60) / 2; }catch(e){}
    const base = hz >= 1 ? hz.toFixed(2) + "Hz  \u00b7  " + Math.round(bpm) + " BPM"
                         : (lfoPeriod(hz) || hz.toFixed(3) + "Hz");
    return base + (hz > lim ? "  \u00b7  faster than the frame rate can draw: aliases" : "");
  }
  return lfoPeriod(hz) || "Hz";
}
  ...
  for(const g of groups) for(const id of g.ids){
    ...
    if(m && m.type === "lfo"){
      ...
      const showRate = makeNumField(v1, {
        ...
        fmt: lfoHz,
        after: ()=>{ per.textContent = lfoNote(m); },
      });
      ...
      { const rp = document.createElement("div"); rp.className = "mcrow mcnote";
        ...
        rp.appendChild(per); card.appendChild(rp); card.__lfoNote = per; }
      ...
      attachTip(sy, "TEMPO LOCK",
        "Locks this LFO to the tempo instead of to its own rate, in note divisions: 1 BAR is one cycle every four beats, 1/16 is four cycles a beat. The tempo comes from TAP or from MIDI clock, so everything locked moves together and stays together when the tempo changes. FREE gives the rate control back.",
        "Below a thirty-second the division asks for more cycles a second than the render can draw and you get an alias rather than the rate on the label \u2014 a fast strobe reading as a slow crawl. The line under the rate says when that is happening, and it is worth hearing out: it is the same effect as a wheel filmed through a shutter appearing to turn backwards.");
      sy.onchange = ()=>{
        const was = m.sync || 0;
        m.sync = parseFloat(sy.value);
        s1.disabled = v1.disabled = m.sync > 0;
        if(m.sync > 0){
          /* keep the free rate for when it is let go again, so switching to a
             division and back is not a way to lose the rate you had set */
          if(!(was > 0)) m.freeRate = m.rate;
          m.rate = (bpm/60)/m.sync;
        } else {
          const lo = Math.pow(10, LFO_LO), hi = Math.pow(10, LFO_HI);
          const back = (m.freeRate > 0) ? m.freeRate : m.rate;
          m.rate = Math.min(hi, Math.max(lo, back));
          s1.value = Math.log10(m.rate);
        }
        showRate();
      };
    ...
    modCards[id] = {cv, ctx:cv.getContext("2d"), val, dests, card,
                    lfoNoteEl: card.__lfoNote || null, lfoMod: (m && m.type === "lfo") ? m : null};
  }
}
const modMacroRefs = {};
...
function drawModPage(){
  ...
  for(const id in modCards){
    ...
    /* a tapped tempo moves every locked rate, so the note is redrawn with the
       rest of the card rather than only when the selector is touched */
    if(c.lfoNoteEl && c.lfoMod){
      const want = lfoNote(c.lfoMod);
      if(c.lfoNoteEl.textContent !== want) c.lfoNoteEl.textContent = want;
    }
