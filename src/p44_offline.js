...
/* The blob handed to the download used to be left for the garbage collector,
   which cannot take it while a URL still refers to it. A session of renders
   therefore held every file it had ever made until the tab closed. The link
   has already been clicked by the time the next render starts, so releasing
   it then is safe and is the last moment it can be done automatically. */
...
async function offlineRender(){
  ...
  if(lastRenderUrl){ try{ URL.revokeObjectURL(lastRenderUrl); }catch(e){} lastRenderUrl = null; }
  ...
  let enc = null, aenc = null, muxer = null;
  const parts = [];
  const suggested = "bendr-"+stamp()+".mp4";
  /* From here to the finally, everything is inside one guard. The cleanup used
     to sit after the frame loop, so anything that threw inside the loop left
     the canvas at the render size, both encoders open and every clip paused,
     and the only sign was a line in the console. A render can fail for reasons
     that are nobody's fault - a decoder giving up on a frame, the disk filling
     - and it has to put the machine back the way it found it when it does. */
  try{
  ...
  if(aBuf){
    try{
      aenc = new AudioEncoder({
        output: (chunk, meta) => { if(muxer) muxer.addAudioChunk(chunk, meta); },
  ...
  /* A seek that never fires "seeked" used to hang the render for good: nothing
     settled the promise, so the loop never came back and CANCEL could not reach
     it either - the flag was set and nothing was ever going to read it again.
     It waits with a ceiling now, and polls the cancel flag while it waits so
     the button works mid-seek. The ceiling is deliberately generous, because a
     real seek into a large file on a slow disk is not quick and cutting one
     short costs a frame. When it is hit the picture captured is whatever the
     decoder happened to be showing, so those frames are counted and said out
     loud at the end rather than passed off as a clean render. */
  const SEEK_LIMIT_MS = 8000;
  let seekOverruns = 0;
  const seekTo = (v,t)=>new Promise(res=>{
    const dur = v.duration;
    const to = (dur && isFinite(dur)) ? Math.min(t, dur-0.001) : t;
    if(renderCancel || Math.abs(v.currentTime - to) < 1e-4) return res();
    let done = false, timer = 0, poll = 0;
    const fin = late=>{
      if(done) return;
      done = true;
      if(late) seekOverruns++;
    ...
    timer = setTimeout(()=>fin(true), SEEK_LIMIT_MS);
    poll = setInterval(()=>{ if(renderCancel) fin(false); }, 50);
    try{ v.currentTime = to; }catch(e){ fin(false); }
  });
  /* a render that limped is still a render, but you should be told it limped */
  const seekNote = ()=> seekOverruns
    ? "  \u00b7  " + seekOverruns + " frame" + (seekOverruns===1?"":"s")
      + " captured before the seek finished"
    : "";
  ...
  for(let i=0; i<total && !renderCancel; i++){
    ...
    vf.close();
    pumpAudio(t + 1);
    /* waiting on the picture alone let the audio encoder build a backlog of its
       own, which is memory nobody is watching */
    while((enc.encodeQueueSize > 6
           || (aenc && aenc.state === "configured" && aenc.encodeQueueSize > 30))
          && !renderCancel) await new Promise(r=>setTimeout(r,5));
    if(i%3===0){
    ...
    if(!renderCancel){
      ...
      if(fileStream){
        ...
        toast("Rendered "+total+" frames \u2192 "+fileHandle.name+" "+audioNote()+seekNote());
      } else {
        ...
        dl(lastRenderUrl, suggested);
        toast("Rendered "+total+" frames \u2192 MP4, "+(blob.size/1048576).toFixed(1)+" MB "+audioNote()+seekNote());
      }
    } else {
    ...
    toast("Render failed: "+String((e && e.message) || e), true);
  }finally{
    ...
    /* play() rejects rather than throws when the browser is not in the mood */
    if(wasPlaying){ try{ video.play().catch(()=>{}); }catch(e){} }
    for(const ch of CHANNELS){
      const S = SRC[ch];
      if(S.video !== video && S.mode === "file" && !S.still){
        try{ S.video.play().catch(()=>{}); }catch(e){}
      }
    }
  }
}
