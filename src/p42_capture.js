...
let recFps = 30, recFrameCount = 0, recLastFrameMs = 0, recDroppedFrames = 0, recBytes = 0, recWarned2G = false;
let lastRecUrl = null;
...
async function startRec(){
  ...
  try{
    recFileStream = fileStream;
    recFileHandle = fileHandle;
    recTarget = fileStream
    ...
    recEnc = new VideoEncoder({
      output: (chunk, meta) => {
        if(recMuxer) recMuxer.addVideoChunk(chunk, meta);
        recBytes += (chunk.byteLength || 0);
        if(!recFileStream && recActive){
          if(recBytes > 3_800_000_000){
            toast("Recording auto-stopped: 3.8 GB memory safety limit reached", true);
            stopRec();
          } else if(recBytes > 2_000_000_000 && !recWarned2G){
            recWarned2G = true;
            toast("Recording reached 2 GB (in-memory mode)", false);
          }
        }
      },
      error: e => {
    ...
    if(hasAudio){
      try{
        recAEnc = new AudioEncoder({
          output: (chunk, meta) => {
            if(recMuxer){
              const tsUs = Math.round(Math.max(0, chunk.timestamp - (recStart * 1000)));
    ...
    if(recFileStream){ try{ await recFileStream.abort(); }catch(_){} }
    recMuxer = null; recTarget = null; recFileStream = null; recFileHandle = null;
    recLocked = false;
    if(recSize){ canvas.width = recSize.w; canvas.height = recSize.h; recSize = null; markSizeDirty(); }
  }
}
async function stopRec(aborted){
  ...
  try{
    if(!aborted && recEnc && recEnc.state === "configured"){
      ...
      const dropNote = recDroppedFrames > 0 ? (", " + recDroppedFrames + " dropped frame" + (recDroppedFrames > 1 ? "s" : "")) : "";
      if(recFileStream){
        await recFileStream.close();
        toast("Recording saved \u2192 " + (recFileHandle ? recFileHandle.name : "disk") + " (" + (recBytes/1048576).toFixed(1) + " MB, " + W + "\u00d7" + H + " @" + fps + "fps"
              + (hadAudio ? ", with audio" : ", no audio \u2014 AUDIO REACT is OFF") + dropNote + ")");
      } else {
      ...
      if(recFileStream){ try{ await recFileStream.abort(); }catch(e){} }
