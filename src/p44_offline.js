...
async function offlineRender(){
  ...
  try {
    ...
    let localActx = null;
    const hasAudioFile = !!(audioFileEl && (audioFileEl.src || audioFileEl.dataset?.url));
    const audioSrcUrl = hasAudioFile
      ? (audioFileEl.src || audioFileEl.dataset.url)
      : ((SRC.A.mode === "file" && (SRC.A.video.src || SRC.A.objUrl)) ? (SRC.A.video.src || SRC.A.objUrl) : null);
    const audioLoop = hasAudioFile
      ? !!(audioFileEl && audioFileEl.loop)
      : !!(SRC.A.video && SRC.A.video.loop);
      ...
      try{
        ...
        if(res.ok){
          ...
          if(ab.byteLength > 100 * 1024 * 1024){
            throw new Error("Audio buffer exceeds 100MB safety limit. Fallback to video-only.");
          }
          ...
          if(audioBuffer && audioBuffer.duration > 600){
            console.warn("Audio track exceeds 10 minute limit (" + (audioBuffer.duration/60).toFixed(1) + " min). Falling back to video-only to prevent OOM crash.");
            audioBuffer = null;
          }
        }
      }catch(e){
    ...
    /* Verify audio format after possible resampling */
    const standardRates = [32000, 44100, 48000];
    if(audioBuffer && (audioBuffer.numberOfChannels > 2 || !standardRates.includes(audioBuffer.sampleRate))){
      console.warn("Audio format unsupported after resampling. Falling back to video-only.");
      audioBuffer = null;
    }
    ...
    const encodeAudioUpTo = (targetFrames, forceFlush = false) => {
      while(audioOffset < targetFrames && !renderCancel){
        const availableFrames = targetFrames - audioOffset;
        if(availableFrames < CHUNK_SIZE && !forceFlush){
          break;
        }
        const framesInChunk = Math.min(CHUNK_SIZE, availableFrames);
        ...
        for(let ch = 0; ch < numChannels; ch++){
          ...
          for(let f = 0; f < framesInChunk; f++){
            const frameIdx = audioOffset + f;
            const idx = (audioLoop && srcLen > 0) ? (frameIdx % srcLen) : frameIdx;
    ...
    for(let i=0; i<total && !renderCancel; i++){
      ...
      for(const ch of ["B","C","D"]){
        ...
        if(srcReady(ch) && v.duration){
          await seekTo(v, t % v.duration);
          if(renderCancel) break;
        }
      }
      if(renderCancel) break;
      ...
      if(hasAudio){
        ...
        encodeAudioUpTo(targetFrames, false);
      }
    ...
    if(!renderCancel){
      ...
      if(hasAudio){
        encodeAudioUpTo(totalAudioFrames, true);
        await aenc.flush();
      }
      await enc.flush();
      muxer.finalize();
