...
const PHELP = {
  ...
  genRate:"How fast the generator animates on its own. Negative runs it backwards. At zero the picture is completely still, which is useful when you want something else to be the only moving thing. Worth knowing if you set it low and the picture still shimmers: that is not the generator. The TAPE / SYNC stage runs on its own whatever the generator is doing, because a tape machine and a sync circuit are never perfectly still, and it is the one thing here that moves without being told to. Switch that stage out at its eye and a frozen generator is genuinely frozen.",
...
/* ---------------- per-section help ---------------- */
const SECHELP = {
  stems:"A track pulled apart into its parts, all playing together, each part driving something different. Load the stems of one song and the kick moves one thing, the bass another, the vocal a third, and because they came out of the same session they are already separated in a way no amount of filtering a finished mix can match. They start together and stay together: each one is decoded into memory and every source in the set is scheduled at a single instant on the audio clock, rather than started at roughly the same moment and nudged towards each other afterwards, which is what drifts. The price is memory, about a megabyte a second for a stereo stem, and the panel says what it is holding. There is no EQ on a stem and that is the point rather than an omission: a band control exists to find the kick inside a mix, and a kick stem is already the kick. Level is only about what you hear, so a part pulled right down still drives whatever it is patched to at full range, which is how you use a stem nobody is meant to listen to. The audio cannot travel in a saved patch, but the shape of the set does: reopen one and the stems are there by name with their routes intact, waiting for the files to be dropped back on.",
  layers:"The other way to wire four channels together. The mixer is a tree: two pictures into a bus, two buses into the master, everything arriving through a fader. That is how a bench mixer is built and it is the right shape for cutting and wiping between sources, but it is the wrong shape for building one picture out of four, because every layer past the second has to come in through a fader that is already carrying something else. The stack is the other topology. Four layers, bottom to top, each landing on everything under it with its own blend mode, its own level and its own key, and a crosspoint matrix deciding which channel feeds which layer. Each layer is a row: its blend, its key and its level, with the threshold, softness and hue appearing beside them only once a key is chosen. The same channel can feed more than one layer, which is how you put a picture over a treated copy of itself. Switching to LAYERS leaves the three transitions dimmed but intact: nothing is lost, and the master melt still runs on the finished stack, so the whole thing can still feed back into itself.",
  ...
  glsl:"A fragment shader as the channel's picture, written to the mainImage convention almost every published shader uses. The rest of the rig sits downstream of it, and iChannel0 can be another channel or the finished programme, so it is equally a processing stage. iChannel1 is always the shader's own previous frame.",
